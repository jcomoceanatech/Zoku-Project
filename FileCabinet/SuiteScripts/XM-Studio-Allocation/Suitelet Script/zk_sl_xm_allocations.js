    ...
    define(['N/record', 'N/render','N/file','N/search','N/ui/serverWidget', 'N/task', 'N/runtime', 'N/https','N/format', "../Library/js/handlebars-v4.7.7", "N/url", "../Library/zk_xm_library", '../Library/js/moment'],
        function(record, render,file,search,serverWidget, task, runtime,https, format, handlebar, url, libHelper, moment) {
            ...
            function submitAllocationOrder(context) {
                ...
                var stPreorderDate = (context.request.parameters.txtCreateAllocationPreorderDate) ? format.parse({value: moment(context.request.parameters.txtCreateAllocationPreorderDate).format(stDatePreference), type: format.Type.DATE}) : "";
                var stFirstOrderDeadline = (context.request.parameters.txtCreateAllocationFirstOrderDeadline) ? format.parse({value: moment(context.request.parameters.txtCreateAllocationFirstOrderDeadline).format(stDatePreference), type: format.Type.DATE}) : "";

                var recProductAllocation = record.create({ type:"customrecord_zk_product_allocation", isDynamic:true });
                recProductAllocation.setValue({fieldId: libHelper.PRODUCT_ALLOCATION_RECORD.DISTRIBUTOR, value:stDistributor});
                ...
                recProductAllocation.setValue({fieldId: libHelper.PRODUCT_ALLOCATION_RECORD.PREORDER_DATE, value:stPreorderDate});
                recProductAllocation.setValue({fieldId: libHelper.PRODUCT_ALLOCATION_RECORD.FIRSTORDER_DEADLINE, value:stFirstOrderDeadline});
