...
define(['N/currentRecord', 'N/record', 'N/ui/dialog', 'N/search', 'N/email'], function (currentRecord, record, dialog, search, email) {
    ...
    function changeAcknowledgeStatus() {
        try {
            ...
            if(!objProductAllocation.department && !objProductAllocation.class) { errorMessage += "No data available for item department or class fields.\n"; }
            // if(!objProductAllocation.pricinggroup) { errorMessage += "No Pricing Group Setup under the Item Record.\n"; }
            if(!objProductAllocation.pricelevel) { errorMessage += "No Price Level Setup under the Customer Record.\n"; }
            if(!fetchItemRate) { errorMessage += "No Price Setup for "+objProductAllocation.priceleveltext+".\n"; }
            if(!objProductAllocation.advanceitem) { errorMessage += "No Advance Item Setup under the Item Record.\n"; }
    ...
    function createSalesOrder(objProductAllocation) {
        ...
        currentLine.setSublistValue({ sublistId: 'item', fieldId: 'item', line: 0, value: objProductAllocation.custrecord_zk_pa_item });
        ...
        currentLine.setSublistValue({ sublistId: 'item', fieldId: 'custcol_discount_percent', line: 0, value: flDiscount });
        currentLine.setSublistValue({ sublistId: 'item', fieldId: 'custcol_discount_amount', line: 0, value: flDiscountAmount });
